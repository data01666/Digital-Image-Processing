...
void EDLines::JoinCollinearLines() {
    ...
    while (i < linesNo) {
        ...
        std::vector<std::pair<double, double>> mergedKeyPoints; // 存储合并后的关键点

        // 将当前线段的起点加入关键点
        mergedKeyPoints.push_back({lines[i].sx, lines[i].sy});
        ...
        for (int j = i + 1; j < linesNo; j++) {
            ...
            // 如果可以合并线段，加入关键点
            if (TryToJoinTwoLineSegments(&lines[lastLineIndex], &lines[j], lastLineIndex)) {
                // 将合并线段的关键点加入，并且防止重复加入终点
                mergedKeyPoints.push_back({lines[j].sx, lines[j].sy});
            } else {
                lastLineIndex++;
                if (lastLineIndex != j)
        ...
        // 将最后的终点加入关键点
        mergedKeyPoints.push_back({lines[lastLineIndex].ex, lines[lastLineIndex].ey});

        // 确保关键点数量不超过5个，并选择代表性点
        if (mergedKeyPoints.size() > 5) {
            std::vector<std::pair<double, double>> limitedKeyPoints;
            int totalPoints = mergedKeyPoints.size();
            for (int k = 0; k < 5; ++k) {
                // 根据比例选取点
                double ratio = (double)k / (5 - 1);  // 计算比例
                int selectedIndex = std::round(ratio * (totalPoints - 1));
                limitedKeyPoints.push_back(mergedKeyPoints[selectedIndex]);
            }
            mergedKeyPoints = limitedKeyPoints; // 更新为限制后的关键点
        }

        // 将合并后的关键点加入最后的线段
        lines[lastLineIndex].keyPoints = mergedKeyPoints;

        // 如果段的第一个线段与最后一个线段成功合并
...
void EDLines::writeAllLineSegmentsToFile(const std::string &filename) {
	// 创建输出文件流
	...
	for (int i = 0; i < linesNo; i++) {
		...
		// 写入线段的起点和终点
		outFile << "Line " << i + 1 << ":\n";
		outFile << "  Start Point: (" << line.sx << ", " << line.sy << ")\n";
		outFile << "  End Point: (" << line.ex << ", " << line.ey << ")\n";

		// 写入线段的段号、像素索引及长度
		outFile << "  Segment No: " << line.segmentNo << "\n";
	...
	// 关闭文件
	outFile.close();
	std::cout << "Line segment information written to " << filename << std::endl;
}
